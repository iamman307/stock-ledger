...
- 證券戶現金對帳新增「轉至幣安」，一次輸入轉帳後銀行餘額、本次轉出金額與圈存。
- 內部轉帳會同步增加波段內加密配置並更新銀行快照，不新增投資本金、不列入損益，也不刪除既有待交割交易。
- 證券戶卡新增「累計轉至幣安」與最近轉出日，並保留裝置端轉出紀錄供後續核對。
- 例如原加密配置15萬元再轉10萬元後，會顯示加密配置25萬元、股票波段基礎額度45萬元。
