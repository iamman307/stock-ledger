...
function renderDash(c){
 ...
 const partial=long.missingQuotes+swing.missingQuotes,cashAdjustment=f.cashAdjustmentTwd,allocatedAvailable=long.available+swing.available,investmentAvailable=f.investmentAvailable;
 ...
 if($('fundOverview'))$('fundOverview').innerHTML=`
   ...
   <div class="hero-metrics"><div><span>規劃內估算資產</span><b>${M(totalTracked)} TWD</b></div><div><span>相對規劃損益</span><b class="${C(totalGain)}">${totalGain>=0?'+':''}${M(totalGain)} TWD</b></div><div><span>${primaryCashLabel}</span><b>${M(primaryCash)} TWD</b></div><div><span>外部投資轉出</span><b class="${C(-f.externalFundingTwd)}">${f.externalFundingTwd?'-':''}${M(f.externalFundingTwd)} TWD</b></div></div>
 ...
 const modelBridge=`<div class="reconcile-bridge"><div><span>長期＋波段策略池</span><b>${M(allocatedAvailable)}</b></div><div><span>＋ 歷史現金校正</span><b class="${C(cashAdjustment)}">${cashAdjustment>=0?'+':''}${M(cashAdjustment)}</b></div><div><span>－ 外部投資轉出</span><b class="neg">-${M(f.externalFundingTwd)}</b></div><div class="bridge-total"><span>＝ 股票配置模型</span><b>${M(investmentAvailable)} TWD</b></div><small>共用校正與外部投資尚未分攤到長期或波段卡片，因此上方兩池相加不會直接等於銀行餘額。</small></div>`;
...
async function refreshAllQuotes(showAlert=false){
  ...
  try{
    ...
    db.meta=db.meta||{};
    db.meta.lastQuoteSnapshot=snapshot.updated||new Date().toISOString();db.meta.appVersion='6.11.0';
