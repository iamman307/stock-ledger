...
function renderDash(c){
 ...
 if($('fundOverview'))$('fundOverview').innerHTML=`
   ...
   <div class="hero-metrics"><div><span>規劃內估算資產</span><b>${M(totalTracked)} TWD</b></div><div><span>相對規劃損益</span><b class="${C(totalGain)}">${totalGain>=0?'+':''}${M(totalGain)} TWD</b></div><div><span>投資可動用現金</span><b>${M(investmentAvailable)} TWD</b></div><div><span>未分配／其他現金</span><b class="${C(cashAdjustment)}">${cashAdjustment>0?'+':''}${M(cashAdjustment)} TWD</b></div></div>
...
async function refreshAllQuotes(showAlert=false){
  ...
  try{
    ...
    db.meta=db.meta||{};
    db.meta.lastQuoteSnapshot=snapshot.updated||new Date().toISOString();db.meta.appVersion='6.9.1';
