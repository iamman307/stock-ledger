const KEY='stock-ledger-v2-preloaded';const DEFAULT_DB={"transactions":[],"manualTrades":[],"quotes":{},"cash":{"長期":0,"波段":0,"loan":0,"reserve":0},"meta":{"appVersion":"6.5.1","publicSafe":true}};let storageBlocked=false;let db;try{db=Ledger.validate(JSON.parse(localStorage.getItem(KEY)||JSON.stringify(DEFAULT_DB)));}catch(err){storageBlocked=true;db=JSON.parse(JSON.stringify(DEFAULT_DB));setTimeout(()=>alert('資料無法讀取，原始資料未改動。請先匯出原始資料，再使用完整還原。'+err.message),0);}if(!Array.isArray(db.manualTrades))db.manualTrades=[];db.meta=db.meta||{};db.meta.symbolMap=db.meta.symbolMap||{};
...
function save(){if(storageBlocked)throw Error('資料讀取異常，禁止覆寫；請使用完整還原');db=Ledger.validate(db);localStorage.setItem(KEY,JSON.stringify(db));renderAll()}
...
function commitDb(next){if(storageBlocked)throw Error('請先完整還原');const checked=Ledger.validate(next);snapshot();localStorage.setItem(KEY,JSON.stringify(checked));db=checked;renderAll();}
...
$('txForm').onsubmit=e=>{e.preventDefault();const t={tax:N($('tax').value),id:crypto.randomUUID(),date:$('date').value,account:$('account').value,ticker:$('ticker').value.trim().toUpperCase(),asset:$('asset').value,side:$('side').value,qty:N($('qty').value),price:N($('price').value),fee:N($('fee').value),fx:N($('fx').value)||1,stop:$('stop').value===''?null:N($('stop').value),currency:$('currency').value,exitReason:$('exitReason').value,note:$('note').value.trim()};if(!t.ticker||t.qty<=0)return alert('請確認代號與數量');try{const next=JSON.parse(JSON.stringify(db));next.transactions.push(t);const checked=Ledger.validate(next);const c=Ledger.compute(checked);if(c.issues.length)throw Error(c.issues.join('\n'));commitDb(checked);}catch(err){return alert(err.message)}e.target.reset();$('fee').value=0;$('fx').value=db.meta?.currentUsdTwd||1;$('currency').value='USD';setNow()};
...
async function refreshAllQuotes(showAlert=false){
  ...
  try{
    ...
    db.meta=db.meta||{};
    db.meta.lastQuoteSnapshot=snapshot.updated||new Date().toISOString();db.meta.appVersion='6.5.1';
...
$('importJson').onchange=async e=>{const f=e.target.files[0];if(!f)return;try{
 const incoming=JSON.parse(await f.text()),{db:next,report:r}=Ledger.merge(db,incoming);
...
$('restorePreImport').onclick=()=>{
  ...
  try{const next=Ledger.validate(JSON.parse(raw));localStorage.setItem(KEY,JSON.stringify(next));db=next;storageBlocked=false;renderAll();alert('已還原到最近一次匯入前狀態。')}
  catch{alert('快照損壞，無法還原。')}
};

$('saveCash').onclick=()=>{try{const next=JSON.parse(JSON.stringify(db));next.cash={'長期':N($('cashLong').value),'波段':N($('cashSwing').value),loan:N($('cashLoan').value),reserve:N($('cashReserve').value)};commitDb(next)}catch(err){alert(err.message)}};function renderCash(){$('cashLong').value=N(db.cash['長期']);$('cashSwing').value=N(db.cash['波段']);$('cashLoan').value=N(db.cash.loan);$('cashReserve').value=N(db.cash.reserve)}function renderAll(){const c=compute();$('dataWarnings').textContent=c.issues.join('；')||'資料檢查：'+db.transactions.length+' 筆股票交易 / '+db.manualTrades.length+' 筆歷史交易。已實現總額僅含股票；持倉缺行情時未實現為部分估值。';renderDash(c);renderTx();renderPos(c);renderPerf(c);renderManualTrades();renderQuotes();renderCash();if($('fxDash'))$('fxDash').innerHTML=db.meta?.currentUsdTwd?`目前 USD/TWD：<b>${F(db.meta.currentUsdTwd,4)}</b>｜最後更新：${db.meta.fxUpdated?new Date(db.meta.fxUpdated).toLocaleString('zh-TW'):'—'}｜歷史成交匯率保持不變。`:'目前匯率尚未成功自動更新；歷史成交匯率保持不變。'}
...
$('restoreJson').onchange=async e=>{const f=e.target.files[0];if(!f)return;try{const next=Ledger.validate(JSON.parse(await f.text()));const c=Ledger.compute(next);if(c.issues.length)throw Error(c.issues.join('\n'));if(!confirm('完整還原將取代目前全部交易、歷史交易、資金池與行情。\n還原檔：'+next.transactions.length+' 筆股票、'+next.manualTrades.length+' 筆歷史。\n請先匯出現有備份。確定？'))return;snapshot();localStorage.setItem(KEY,JSON.stringify(next));db=next;storageBlocked=false;renderAll();alert('完整還原完成');}catch(err){alert('未還原：'+err.message)}finally{e.target.value='';}};
