...
function updateSettlementOption(resetDefault=false){const box=$('pendingSettlement'),hint=$('settlementHint'),side=$('side').value,cash=Ledger.securitiesCashSummary(db),eligible=side!=='INIT'&&cash.configured;box.disabled=!eligible;if(resetDefault)box.checked=eligible;else if(!eligible)box.checked=false;if(hint)hint.textContent=!cash.configured?'需先匯入證券戶銀行快照，才能自動建立待交割款。':side==='INIT'?'期初持倉不會建立待交割款。':'只適用於尚未從銀行扣款／入帳的新成交；回補舊交易請取消勾選。';}
$('side').addEventListener('change',()=>updateSettlementOption(true));updateSettlementOption();
$('txForm').onsubmit=e=>{e.preventDefault();const ticker=$('ticker').value.trim().toUpperCase();const t={tax:N($('tax').value),id:crypto.randomUUID(),date:$('date').value,account:Ledger.classifyAccount(ticker),ticker,asset:$('asset').value,side:$('side').value,qty:N($('qty').value),price:N($('price').value),fee:N($('fee').value),fx:N($('fx').value)||1,stop:$('stop').value===''?null:N($('stop').value),currency:$('currency').value,exitReason:$('exitReason').value,note:$('note').value.trim()};if(!t.ticker||t.qty<=0)return alert('請確認代號與數量');try{const next=JSON.parse(JSON.stringify(db));next.transactions.push(t);if($('pendingSettlement').checked){const cash=Ledger.normalizeSecuritiesCash(next.meta?.securitiesCash);if(!cash.enabled||!cash.asOf)throw Error('證券戶銀行快照尚未啟用，請取消「尚未交割」或先匯入銀行快照');cash.pendingSettlements.push(Ledger.pendingSettlementFromTransaction(t));next.meta.securitiesCash=cash;}const checked=Ledger.applyPolicy(next);const c=Ledger.compute(checked);if(c.issues.length)throw Error(c.issues.join('\n'));commitDb(checked);}catch(err){return alert(err.message)}e.target.reset();$('fee').value=0;$('tax').value=0;$('fx').value=db.meta?.currentUsdTwd||1;$('currency').value='USD';setNow();updateAccountHint();updateSettlementOption(true)};
...
function renderDash(c){
 ...
 const mv=long.marketValue+swing.marketValue,cost=long.cost+swing.cost,unr=long.missingQuotes+swing.missingQuotes?NaN:mv-cost,r=long.realized+swing.realized;
 ...
 const partial=long.missingQuotes+swing.missingQuotes,cashAdjustment=f.cashAdjustmentTwd,investmentAvailable=f.investmentAvailable;
 const bankDifference=investmentAvailable-securities.postSettlementTwd;
 const primaryCash=securities.configured?securities.postSettlementTwd:investmentAvailable,primaryCashLabel=securities.configured?'證券戶交割後預估':'股票配置模型餘額';
 if($('fundOverview'))$('fundOverview').innerHTML=`
   ...
   <div class="hero-metrics"><div><span>規劃內估算資產</span><b>${M(totalTracked)} TWD</b></div><div><span>相對規劃損益</span><b class="${C(totalGain)}">${totalGain>=0?'+':''}${M(totalGain)} TWD</b></div><div><span>${primaryCashLabel}</span><b>${M(primaryCash)} TWD</b></div><div><span>波段內加密配置</span><b class="neg">-${M(f.externalFundingTwd)} TWD</b></div></div>
   <p class="pool-note">波段70萬已包含加密交易本金；股票 TWD 與網格／合約 USDT 績效仍分開統計。日常資金與旅遊日圓不納入。</p>
   <details class="pool-note"><summary>查看估算口徑差異</summary>股票交易現金流餘額：${M(long.cashFlowAvailable+swing.cashFlowAvailable)} TWD<br>波段內加密配置：-${M(f.externalFundingTwd)} TWD<br>損益口徑調整：${F(long.reconciliationAdjustment+swing.reconciliationAdjustment)} TWD<br>加密配置是本金用途，不是投資損益。</details>
 ...
 const modelBridge=`<div class="reconcile-bridge"><div><span>長期策略剩餘</span><b>${M(long.available)}</b></div><div><span>波段策略剩餘</span><b>${M(f.swingAvailableTwd)}</b></div><div><span>＋ 歷史現金校正</span><b class="${C(cashAdjustment)}">${cashAdjustment>=0?'+':''}${M(cashAdjustment)}</b></div><div class="bridge-total"><span>＝ 股票配置模型</span><b>${M(investmentAvailable)} TWD</b></div><small>波段策略剩餘已扣除 ${M(f.externalFundingTwd)} 元加密配置，因此不會在此重複扣除。</small></div>`;
 const pendingPanel=`<div class="pending-settlements"><div class="pending-head"><strong>待交割明細（${securities.pendingSettlements.length}）</strong><div class="pending-head-actions"><button class="tiny primary" data-record-external-transfer>轉至幣安</button><button class="tiny" data-update-securities-cash>更新銀行餘額</button></div></div><div class="pending-list">${securities.pendingSettlements.length?securities.pendingSettlements.map(item=>`<div class="pending-row"><div class="pending-copy"><b>${E(item.ticker||'未命名')} · ${item.side==='BUY'?'買進':'賣出'}</b><span>${E(item.date.slice(0,16).replace('T',' '))} · ${F(item.amount,2)} ${E(item.currency)} · 匯率 ${F(item.fx,4)}</span></div><strong class="pending-amount ${C(item.amount*item.fx)}">${item.amount*item.fx>0?'+':''}${M(item.amount*item.fx)} TWD</strong><div class="pending-actions"><button class="tiny primary" data-settle-pending="${E(item.id)}">已交割</button><button class="tiny danger" data-cancel-pending="${E(item.id)}">取消</button></div></div>`).join(''):'<p class="pool-note">目前沒有待交割款。新交易勾選「尚未交割」後會自動出現在這裡。</p>'}</div></div>`;
 const reconcileCard=securities.configured?`<article class="pool-card reconcile-card"><div class="pool-head"><div><span class="pool-kicker">證券戶現金對帳</span><h3>${M(securities.postSettlementTwd)} <small>TWD</small></h3></div><span class="pool-status">銀行快照</span></div><div class="pool-stats"><div><span>帳面餘額</span><b>${M(securities.accountBalanceTwd)}</b></div><div><span>已圈存</span><b class="neg">-${M(securities.reservedTwd)}</b></div><div><span>目前可用</span><b>${M(securities.availableTwd)}</b></div><div><span>待交割估算</span><b class="${C(securities.pendingTwd)}">${securities.pendingTwd>0?'+':''}${M(securities.pendingTwd)}</b></div><div><span>累計轉至幣安</span><b class="neg">-${M(f.externalFundingTwd)}</b></div><div><span>最近轉出</span><b>${securities.externalTransfers.length?E(securities.externalTransfers[securities.externalTransfers.length-1].date.slice(0,10)):'—'}</b></div><div class="wide"><span>全部交割後預估</span><b>${M(securities.postSettlementTwd)} TWD</b></div></div>${pendingPanel}${modelBridge}<div class="pool-stats reconcile-gap"><div><span>模型與銀行待對帳</span><b class="${C(bankDifference)}">${bankDifference>0?'+':''}${M(bankDifference)}</b></div><div><span>判讀</span><b>${Math.abs(bankDifference)<1?'已對平':'不是投資損益'}</b></div></div><p class="pool-note">銀行快照 ${E(securities.asOf)}；「轉至幣安」會同時更新銀行餘額與波段內加密配置，不算新增本金或損益。股票交割仍使用各筆「已交割」。</p></article>`:`<article class="pool-card reconcile-card"><div class="pool-head"><div><span class="pool-kicker">股票配置模型餘額</span><h3>${M(investmentAvailable)} <small>TWD</small></h3></div><span class="pool-status">非銀行餘額</span></div>${modelBridge}<p class="pool-note">這是配置模型，不是銀行存款。匯入證券戶快照後才會顯示帳面餘額、圈存、待交割與交割後預估。</p></article>`;
 $('accountSummary').innerHTML=pools.map(([label,b,klass])=>{const isSwing=label==='波段資金',available=isSwing?f.swingAvailableTwd:b.available,used=Math.max(0,Math.min(100,b.allocation?(b.allocation-available)/b.allocation*100:0)),over=available<0;const extra=isSwing?`<div><span>加密交易配置</span><b class="neg">-${M(f.externalFundingTwd)}</b></div><div><span>股票波段基礎額度</span><b>${M(f.swingStockAllocationTwd)}</b></div>`:'';return `<article class="pool-card ${klass}"><div class="pool-head"><div><span class="pool-kicker">${label}</span><h3>${M(b.allocation)} <small>TWD</small></h3></div><span class="pool-status ${over?'over':''}">${over?'超出配置':'配置內'}</span></div><div class="pool-progress"><span style="width:${used}%"></span></div><div class="pool-stats"><div><span>${isSwing?'波段策略剩餘':'策略預算剩餘'}</span><b class="${over?'neg':''}">${M(available)}</b></div>${extra}<div><span>${isSwing?'股票持倉成本':'目前持倉成本'}</span><b>${M(b.cost)}</b></div><div><span>${isSwing?'已知股票市值':'已知市值'}</span><b>${M(b.marketValue)}</b></div><div><span>${isSwing?'股票已實現損益':'已實現損益'}</span><b class="${C(b.realized)}">${b.realized>=0?'+':''}${M(b.realized)}</b></div></div><div class="ticker-list">${b.tickers.length?b.tickers.map(t=>`<span>${E(t)}</span>`).join(''):'<span class="muted">目前無持倉</span>'}</div><p class="pool-note">${isSwing?'70萬配置已包含加密本金；加密績效在合約／網格頁以 USDT 獨立統計':'自匯入紀錄起估算'}${b.missingQuotes?`；${b.missingQuotes} 檔缺行情`:''}</p></article>`}).join('')+reconcileCard;
...
function renderTx(){const rows=[...db.transactions].sort((a,b)=>D(b.date)-D(a.date));$('txTable').innerHTML=`<thead><tr><th>日期</th><th>帳戶</th><th>代號</th><th>動作</th><th>數量</th><th>價格</th><th>費用</th><th>匯率</th><th>停損</th><th></th></tr></thead><tbody>`+(rows.length?rows.map(t=>`<tr><td>${E(t.date.replace('T',' '))}</td><td>${E(t.account)}</td><td>${E(t.ticker)}</td><td>${E(t.side)}</td><td>${F(t.qty,4)}</td><td>${F(t.price,4)}</td><td>${F(t.fee)}</td><td>${F(t.fx,4)}</td><td>${t.stop==null?'—':F(t.stop,4)}</td><td><button class="tiny danger" data-delete-tx="${E(t.id)}">刪除</button></td></tr>`).join(''):`<tr><td colspan="10" class="empty">尚無交易</td></tr>`)+`</tbody>`}window.delTx=id=>{if(confirm('確定刪除？')){const next=JSON.parse(JSON.stringify(db));next.transactions=next.transactions.filter(x=>x.id!==id);next.meta.securitiesCash.pendingSettlements=next.meta.securitiesCash.pendingSettlements.filter(x=>x.id!=='settlement-'+id);const issues=Ledger.compute(next).issues;if(issues.length)return alert('刪除會造成持股不足，未變更：'+issues.join('\n'));commitDb(next)}};$('clearAll').onclick=()=>{if(confirm('確定清空全部交易？自動建立的待交割款也會一併清除。')){const next=JSON.parse(JSON.stringify(db));next.transactions=[];next.meta.securitiesCash.pendingSettlements=next.meta.securitiesCash.pendingSettlements.filter(x=>!x.id.startsWith('settlement-'));commitDb(next)}};
...
function renderExternalHoldings(){
 ...
 const labels={loan:'信貸',self:'自有',family:'家庭贊助',mixed:'混合資金'};
 ...
 el.innerHTML='<thead><tr><th>資產</th><th>平台</th><th>數量</th><th>平均成本</th><th>快照價格</th><th>市值 TWD</th><th>未實現</th><th>資金註記</th><th>快照日</th></tr></thead><tbody>'+(rows.length?rows.map(h=>{
  ...
  return `<tr><td><b>${E(h.ticker)}</b><br><span class="muted">${E(h.asset)}</span></td><td>${E(h.venue)}</td><td>${F(h.qty,8)}</td><td>${Number.isFinite(h.avgCost)?F(h.avgCost,2)+' '+E(h.currency):'不可考'}</td><td>${F(h.currentPrice,2)} ${E(h.currency)}</td><td>${M(market)}</td><td class="${C(pnl)}">${Number.isFinite(pnl)?(pnl>=0?'+':'')+M(pnl):'不回推'}</td><td>${E(labels[h.source]||'未分類')}${h.legacy?'・歷史期初':''}${h.sourceSummary?'<br><span class="muted">'+E(h.sourceSummary)+'</span>':''}</td><td>${E(h.asOf.slice(0,10))}</td></tr>`;
...
function renderPerf(c){
 ...
 $('tradeTable').innerHTML='<thead><tr><th>標的</th><th>開倉</th><th>平倉</th><th>投入成本 TWD</th><th>實現損益 TWD</th><th>報酬</th><th>R（原幣）</th><th>持有區間</th><th>時間效率（區間估算）</th></tr></thead><tbody>'+stock.map(t=>'<tr><td>'+E(t.ticker)+'</td><td>'+E(t.open)+'</td><td>'+E(t.close)+'</td><td>'+M(t.buyCostTwd)+'</td><td class="'+C(t.realizedTwd)+'">'+M(t.realizedTwd)+'</td><td>'+F(t.returnPct)+'%</td><td>'+(Number.isFinite(t.rMultiple)?F(t.rMultiple,4)+'R':'N/A')+'</td><td>'+F(t.holdHours/24,2)+' d'+(t.estimated?'（期初快照）':'')+'</td><td>'+(t.holdHours>0&&!t.estimated?F(t.returnPct/(t.holdHours<24?t.holdHours:t.holdHours/24),4)+(t.holdHours<24?'%/hour':'%/day'):'N/A')+'</td></tr>').join('')+'</tbody>';
}
...
function renderManualTrades(){
  ...
  (tr.length?tr.map(x=>{const h=(D(x.close)-D(x.open))/36e5,hold=h<24?F(h,2)+' hr':F(h/24,2)+' d';
    const mode=x.strategyMode==='NEUTRAL'?'中性網格':x.strategyMode==='LONG_UPWARD_TRAILING'?'做多網格／向上追蹤':x.strategyMode==='LONG'?'做多網格':x.direction;
    return `<tr><td>${E(x.ticker)}</td><td>${E(mode)}</td><td>${E(x.open.replace('T',' '))}</td><td>${E(x.close.replace('T',' '))}</td><td>${F(x.entry,4)}</td><td>${F(x.exit,4)}</td><td>${F(x.qty,4)}</td><td class="${C(x.pnl)}">${Number.isFinite(x.pnl)?F(x.pnl,2):'N/A'} ${E(x.currency)}</td><td class="${C(x.returnPct)}">${Number.isFinite(x.returnPct)?F(x.returnPct,2)+'%':'N/A'}</td><td class="${C(x.priceReturnPct)}">${Number.isFinite(x.priceReturnPct)?F(x.priceReturnPct,4)+'%':'N/A'}</td><td>${hold}</td></tr>`
...
async function refreshAllQuotes(showAlert=false){
  ...
  try{
    ...
    db.meta=db.meta||{};
    db.meta.lastQuoteSnapshot=snapshot.updated||new Date().toISOString();db.meta.appVersion='6.11.1';
...
function renderCapitalSettings(c){
 ...
 if(!capitalFormDirty){
  ...
  $('capitalInterestPaid').value=profile.interestPaid;$('capitalExcludedDaily').value=profile.excludedDailyTwd;$('capitalCashAdjustment').value=profile.cashAdjustmentTwd;$('capitalOtherPnl').value=profile.otherPnlTwd;
 }
 const status=$('capitalSettingsSummary');
 status.innerHTML=summary.configured?`<div><span>重置日</span><b>${E(profile.resetDate)}</b></div><div><span>目前來源份額</span><b>${M(summary.sourceTotal)} TWD</b></div><div><span>生活資金排除</span><b>${M(profile.excludedDailyTwd)} TWD</b></div><div><span>歷史模型校正</span><b class="${C(profile.cashAdjustmentTwd)}">${profile.cashAdjustmentTwd>0?'+':''}${M(profile.cashAdjustmentTwd)} TWD</b></div><div><span>剩餘信貸本金</span><b>${M(summary.outstandingPrincipal)} TWD</b></div><div><span>重置後投資損益</span><b class="${C(summary.trackedPnlTwd)}">${summary.trackedPnlTwd>=0?'+':''}${M(summary.trackedPnlTwd)} TWD</b></div>`:'<div class="capital-setup-message">尚未啟用。第一次儲存時會把當下股票損益設為 0 起點，過去不回推。</div>';
...
$('capitalForm').onsubmit=e=>{
 ...
 try{
  ...
  const profile={...current,enabled:$('capitalEnabled').checked,resetDate:$('capitalResetDate').value,
   ...
   interestPaid:N($('capitalInterestPaid').value),excludedDailyTwd:N($('capitalExcludedDaily').value),cashAdjustmentTwd:N($('capitalCashAdjustment').value),otherPnlTwd:N($('capitalOtherPnl').value),scope:'investment-only'};
...
function promptTwd(message,current){const raw=window.prompt(message,String(Math.round(current)));if(raw===null)return null;const value=Number(String(raw).replaceAll(',','').trim());if(!Number.isFinite(value)||value<0)throw Error('請輸入正確的非負台幣金額');return value;}
function updateSecuritiesCashFromBank(settleId=''){
 try{
  const cash=Ledger.normalizeSecuritiesCash(db.meta?.securitiesCash);if(!cash.enabled||!cash.asOf)throw Error('尚未建立證券戶銀行快照');
  const item=settleId?cash.pendingSettlements.find(x=>x.id===settleId):null;if(settleId&&!item)throw Error('找不到這筆待交割款，可能已經處理');
  const balance=promptTwd(settleId?'請輸入銀行扣款／入帳後的證券戶「帳面餘額」：':'請輸入銀行目前顯示的證券戶「帳面餘額」：',cash.accountBalanceTwd);if(balance===null)return;
  const defaultReserved=settleId&&cash.pendingSettlements.length===1?0:Math.min(cash.reservedTwd,balance);
  const reserved=promptTwd('請輸入目前仍被圈存／暫不可用的金額；沒有就填 0：',defaultReserved);if(reserved===null)return;if(reserved>balance)throw Error('圈存金額不可大於帳面餘額');
  const next=JSON.parse(JSON.stringify(db)),snapshot=Ledger.normalizeSecuritiesCash(next.meta.securitiesCash);snapshot.accountBalanceTwd=balance;snapshot.reservedTwd=reserved;snapshot.asOf=new Date().toISOString();if(settleId)snapshot.pendingSettlements=snapshot.pendingSettlements.filter(x=>x.id!==settleId);next.meta.securitiesCash=snapshot;commitDb(next);alert(settleId?'已用銀行實際餘額完成交割，不會新增或扣減投資本金。':'證券戶銀行快照已更新。');
 }catch(err){alert(err.message)}
}
function recordExternalInvestmentTransfer(){
 try{
  const cash=Ledger.normalizeSecuritiesCash(db.meta?.securitiesCash);if(!cash.enabled||!cash.asOf)throw Error('尚未建立證券戶銀行快照');
  const balance=promptTwd('請輸入轉帳後，銀行目前顯示的證券戶「帳面餘額」：',cash.accountBalanceTwd);if(balance===null)return;
  const suggested=Math.max(0,cash.accountBalanceTwd-balance);
  const amount=promptTwd('請輸入這次從證券戶轉至幣安的台幣金額：',suggested);if(amount===null)return;if(amount<=0)throw Error('轉出金額必須大於 0');
  const reserved=promptTwd('請輸入目前仍被圈存／暫不可用的金額；沒有就填 0：',Math.min(cash.reservedTwd,balance));if(reserved===null)return;if(reserved>balance)throw Error('圈存金額不可大於帳面餘額');
  const total=Math.max(cash.externalInvestmentTransfersTwd,Ledger.externalFundingTwd(db))+amount;
  if(!confirm(`確認記錄內部轉帳？\n\n本次轉至幣安：${M(amount)} TWD\n證券戶帳面餘額：${M(balance)} TWD\n累計加密配置：${M(total)} TWD\n\n不會新增投資本金，也不會計入損益；既有待交割明細會保留。`))return;
  const id=globalThis.crypto?.randomUUID?crypto.randomUUID():'external-transfer-'+Date.now(),date=new Date().toISOString();
  const next=Ledger.recordExternalInvestmentTransfer(db,{id,date,amountTwd:amount,accountBalanceTwd:balance,reservedTwd:reserved,destination:'Binance',note:'證券交割戶轉至幣安'});
  commitDb(next);alert(`已同步更新：幣安累計配置 ${M(total)} 元；這筆是內部轉帳，不是新增投入或損益。`);
 }catch(err){alert(err.message)}
}
function cancelPendingSettlement(id){if(!confirm('只取消這筆待交割追蹤，不會刪除股票交易。確定？'))return;try{const next=JSON.parse(JSON.stringify(db));next.meta.securitiesCash.pendingSettlements=next.meta.securitiesCash.pendingSettlements.filter(x=>x.id!==id);commitDb(next)}catch(err){alert(err.message)}}

function renderAll(){const c=compute();$('dataWarnings').textContent=c.issues.join('；')||'資料正常｜'+db.transactions.length+' 筆股票交易・'+db.manualTrades.length+' 筆其他歷史交易';$('dataWarnings').classList.toggle('has-warning',c.issues.length>0);renderDash(c);renderTx();renderPos(c);renderPerf(c);renderManualTrades();renderQuotes();renderCash();renderCapitalSettings(c);updateSettlementOption();if($('fxDash'))$('fxDash').innerHTML=db.meta?.currentUsdTwd?`USD/TWD <b>${F(db.meta.currentUsdTwd,4)}</b><span>更新 ${db.meta.fxUpdated?new Date(db.meta.fxUpdated).toLocaleString('zh-TW'):'—'}・歷史成交匯率不變</span>`:'USD/TWD 尚未成功更新；歷史成交匯率保持不變。'}
$('perfScope').onchange=()=>renderAll();
...
$('accountSummary').addEventListener('click',e=>{const settled=e.target.closest('[data-settle-pending]'),cancelled=e.target.closest('[data-cancel-pending]'),updated=e.target.closest('[data-update-securities-cash]'),external=e.target.closest('[data-record-external-transfer]');if(settled)updateSecuritiesCashFromBank(settled.dataset.settlePending);else if(cancelled)cancelPendingSettlement(cancelled.dataset.cancelPending);else if(external)recordExternalInvestmentTransfer();else if(updated)updateSecuritiesCashFromBank()});
